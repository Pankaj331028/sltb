...
$name = "intake_comment_result[" . $ansR['intake_comment_id'] . "]";
			...
			if ($intake_question_id == 56) {$input_type = 'email';} elseif ($intake_question_id == 50) {$input_type = 'date';} else { $input_type = 'text';}
			...
			if ($intake_question_id == 49) {echo '<span>(Format as ###-##-####)</span>';}
			?>
</div>
<?php
...
function check_question_9(val)
{
	if(val == '60')
	{
		$("#intake_form_group_56 input[type=email]").attr("required","required");
		$("#intake_form_group_56, #intake_form_required_56").show('100');
	}
	else
	{
		$("#intake_form_group_56 input[type=email]").removeAttr("required");
		$("#intake_form_group_56, #intake_form_required_56").hide('100');
	}
}
