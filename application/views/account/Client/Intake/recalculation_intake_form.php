...
if (isset($client_data['intake_client_status']['status'])) {if ($client_data['intake_client_status']['status'] == "Complete") {base_url('account/intake/recalculation');}}
...
$name = "intake_comment_result[" . $ansR['intake_comment_id'] . "]";
			...
			if ($intake_question_id == 162) {$input_type = 'email';} elseif ($intake_question_id == 156) {$input_type = 'date';} else { $input_type = 'text';}
			...
			if ($intake_question_id == 155) {echo '<span>(Format as ###-##-####)</span>';}
			?>
</div>
<?php
...
function check_question_9(val)
{
	if(val == '139')
	{
		$("#intake_form_group_162 input[type=email]").attr("required","required");
		$("#intake_form_group_162, #intake_form_required_162").show('100');
	}
	else
	{
		$("#intake_form_group_162 input[type=email]").removeAttr("required");
		$("#intake_form_group_162, #intake_form_required_162").hide('100');
	}
}
