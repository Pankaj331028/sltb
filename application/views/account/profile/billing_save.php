...
<div class="container">
	<div class="panel">
		<div class="panel-heading">
			<h3 class="panel-title">Update Billing Details</h3>


			<!-- Product Info -->
			<!-- <p><b>Amount:</b> <?php echo '$' . round($payment_data['price'], 2) . ' ' . $payment_data['currency']; ?></p> -->
		</div>
		<div class="panel-body">
        	<!-- Display errors returned by createToken -->
			<div id="paymentResponse"><?php if ($this->session->flashdata('error')): echo '<p>' . $this->session->flashdata('error') . '</p>';
	$this->session->set_flashdata('error', '');endif;?></div>

			<!-- Payment form -->
			<form action="" method="post" id="paymentFrm">
				<input type="hidden" name="type_account" id="name" class="field" value="<?php echo $type_account; ?>" >
				<input type="hidden" name="privacy_policy" id="name" class="field" value="<?php echo $privacy_policy; ?>" >
            <!-- paymentFrm -->
				<div class="form-group">
					<label>NAME</label>
					<input type="text" name="name" id="name" class="field" value="<?php echo $GLOBALS["loguser"]["name"]; ?>" required="" autofocus="">
				</div>
				<!-- <div class="form-group">
					<label>EMAIL</label>
					<input type="email" name="email" id="email" class="field" value="<?php echo $GLOBALS["loguser"]["email"]; ?>" required="">
				</div> -->
				<div class="form-group">
				...
				<button type="submit" class="btn btn-success" id="payBtn">Save Card</button>
                <p><a href="<?php echo base_url('account/billing'); ?>">Back to Billing</a></p>
			</form>
		</div>
	</div>
</div>
...
// const resultContainers = document.getElementById('card-result');
const resultContainers = document.getElementById('paymentResponse');
let paymentMethodId;
let cardnumber;
...
async function handlePayment() {
  ...
  if (error) {
	resultContainers.innerHTML = '<p>'+error.message+'</p>';
...
function stripeTokenHandler(token) {
	...
	hiddenInput.setAttribute('value', cardnumber);
	// hiddenInput.setAttribute('paymentMethod_id', resultContainers.textContent);
	form.appendChild(hiddenInput);

	var thiddenInput = document.createElement('input');
	thiddenInput.setAttribute('type', 'hidden');
	thiddenInput.setAttribute('name', 'stripeToken');
	thiddenInput.setAttribute('value', token.id);
	form.appendChild(thiddenInput);


	var hiddenPaymentMethodInput = document.createElement('input');
	hiddenPaymentMethodInput.setAttribute('type', 'hidden');
	hiddenPaymentMethodInput.setAttribute('name', 'paymentMethod_id');
	hiddenPaymentMethodInput.setAttribute('value', paymentMethodId); // You need to define 'resultContainers'
